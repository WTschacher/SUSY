...
######## FOLGENDE Daten BITTE EINGEBEN

# SUSY surrogate analysis of dyadic synchrony
# David Leander Tschacher, 2016
# 2018: wenn plotid = 1 gewählt wird, wird die CCF als file exportiert!
...
# Dateiname
filename = "MEA-topstage-C3-Beethoven.txt"

# Separator der Daten (Leerzeichen, Strichpunkt, Komma)
seperator = " "

# Missings in den Daten
missings = "."

# Existiert die Kopfzeile der Daten? TRUE, FALSE
datahead = TRUE

# hier bezeichnete Spalte 1 wird mit Spalte 2 verglichen (falls automatic = FALSE)
spalte1 = 2
spalte2 = 5

# Epochengroesse (SEGMENT SIZE) in s (Epoche darf nicht kleiner sein als Zeitfenster/fps)
...
# Zeitfenster (WINDOW SIZE) fuer CCF - > maximaler lag in Sekunden (nur ganze Zahlen)
maxlag = 3
maxlag = maxlag * fps

# automatische Iteration? TRUE, FALSE
...
# welche Art plot soll angezeigt werden (man kann den plot auch manuell ausfuehren (unten))
# 0 nichts, 1 GMcrosscorrs, 2 synchrony by epoch, 3 GM-Z, 4 Zeitreihenplot, 5 z not abs
# wenn plotid2 == 0, dann kein zweiter plot
plotid = 3
plotid2 = 5

# Berechnung der pseudo-Daten vereinfachen (TRUE, FALSE)
...
anzahlPseudosGesamt = 500

# Zufallsdaten (Normalverteilte Zahlen im Bereich [0,50]
zufallsdaten = FALSE
anzahlzufallsdaten = 10000

# Korrelationen ohne Betrag auch anzeigen? (FALSE/TRUE)
korrOhneBetragAnzeigen = TRUE

# Permuation: Synchronie f?r n(n-1)/2 Dyaden berechnen   
permutation = FALSE

######## ENDE DER EINGABEN

data<-read.csv(file=filename,head=datahead,sep=seperator,na.strings=missings)

if (permutation) {
  list_combination<-combinations(n=ncol(data),r=2,v=colnames(data),set=TRUE,repeats.allowed=F)
...
while (numberColumns >= 2) {
	...
	if (pseudosVereinfachen) {
		...
		if (anzahlPseudosProEpoche < 1){
			cat("error: anzahlPseudosProGesamt < numberEpochen, automatisch auf min vergrössert\n")
	...
	#meanccorr mit nullen füllen, länge = lagtimes2
...
dev.new()
	...
	if (firstloop) {
		...
		cat("ES(lead1) ") #CCF wenn der erste (spalte 1) Zahlenstrang führend ist
		cat("ES(lead2) ") #CCF wenn der zweite (spalte 2) Zahlenstrang führend ist
		if (korrOhneBetragAnzeigen) {
			...
			cat("Z(noAbs) ") #25
			cat("Z(Pseudo-noAbs) ")	
			cat("%>Pseudo(noAbs) ") # % meanccorrRealZNotAbs die grösser als meanccorrRealZNotAbs sind
			cat("ES(noAbs) ") #effektstärke
		}
		cat("\n")
		firstloop = FALSE
	}
